      ...
      >
        ...
        <button
          ...
          onClick={() =>
            (props.onMoveEntity as
              | undefined
              | ((entityId: string, position: { x: number; y: number }) => void))?.("board-2", {
              x: 4.3346,
              y: 2.4106,
            })
          }
        >
          Move board near board endpoint
        </button>
        <button
          type="button"
          onClick={() =>
        ...
        >
          Move board away from board endpoint
        </button>
...
function readLatestSmoothArcEntity() {
  const entity = readLatestWorkspaceProps()?.entities?.find(
    (candidate) => candidate.kind === "arc-track" && String(candidate.id).startsWith("smooth-arc-"),
  );

  if (!entity) {
    throw new Error("expected latest workspace smooth arc entity");
  }
...
function readLatestEntity(entityId: string) {
  const entity = readLatestWorkspaceProps()?.entities?.find(
    (candidate) => candidate.id === entityId,
  );

  if (!entity) {
    throw new Error(`expected latest workspace entity ${entityId}`);
  }
...
describe("App direct manipulation contracts", () => {
  ...
  it("previews and commits a managed smooth arc when board endpoints are dragged together", async () => {
    render(<App />);

    fireEvent.click(screen.getByRole("button", { name: "Start board drag" }));
    fireEvent.click(screen.getByRole("button", { name: "Hover stage" }));
    fireEvent.pointerUp(window);

    expect(screen.getByTestId("scene-tree-item-board-2")).toBeDefined();

    fireEvent.click(screen.getByTestId("scene-tree-item-board-2"));
    fireEvent.change(screen.getByLabelText("Angle"), { target: { value: "-30" } });

    expect(readLatestEntity("board-2")).toMatchObject({
      rotationDegrees: -30,
    });

    fireEvent.click(screen.getByRole("button", { name: "Move board near board endpoint" }));

    await waitFor(() => {
      const preview = readLatestWorkspaceProps()?.authoringPlacementPreview;

      expect(preview?.status).toBe("snap");
      expect(preview?.entity?.kind).toBe("arc-track");
    });
    expect(screen.queryByTestId("scene-tree-item-smooth-arc-board-1-end-board-2-start")).toBeNull();

    fireEvent.mouseUp(window);

    await waitFor(() =>
      expect(readLatestWorkspaceProps()?.entities?.map((entity) => entity.id)).toContain(
        "smooth-arc-board-1-end-board-2-start",
      ),
    );
    expect(readLatestSmoothArcEntity()).toMatchObject({
      autoGenerated: true,
      anchorEntityId: "board-1",
      anchorEndpoint: "end",
      managedConnection: {
    ...
    const board1 = readLatestEntity("board-1");
    const board2 = readLatestEntity("board-2");

    if (board1.kind !== "board" || board2.kind !== "board") {
      throw new Error("expected managed smooth arc test boards to remain boards");
    }

    const board1End = getBoardArcEndpoints(board1).end.point;
    const board2Start = getBoardArcEndpoints(board2).start.point;

    expect(board1End.x).toBeCloseTo(board2Start.x, 4);
    expect(board1End.y).toBeCloseTo(board2Start.y, 4);
  });

  it("removes a managed smooth arc after connected board endpoints are dragged apart", async () => {
    render(<App />);
    ...
    fireEvent.click(screen.getByRole("button", { name: "Move board away from board endpoint" }));
    fireEvent.mouseUp(window);

    expect(
      screen.queryByTestId("scene-tree-item-smooth-arc-board-1-end-board-2-start"),
    ).toBeNull();
  });
