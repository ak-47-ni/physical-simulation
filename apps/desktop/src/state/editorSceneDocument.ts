...
function mapEditorEntityToSceneEntity(entity: EditorSceneEntity): SceneEntity {
  if (entity.kind === "arc-track") {
    return {
      ...
      ...(entity.autoGenerated !== undefined ? { autoGenerated: entity.autoGenerated } : {}),
      id: entity.id,
      kind: "arc-track",
      label: entity.label,
    ...
    return [
      {
        ...
        ...(readBoolean(entity, "autoGenerated") !== undefined
          ? { autoGenerated: readBoolean(entity, "autoGenerated") }
          : {}),
        id: entity.id,
        kind: "arc-track",
        ...
        rotationDegrees: entity.rotationDegrees,
        ...(readManagedConnection(entity)
          ? { managedConnection: readManagedConnection(entity) }
          : {}),
...
function readBoolean<T extends object, K extends string>(
  value: T,
  key: K,
): boolean | undefined {
  const candidate = (value as Record<string, unknown>)[key];
...
function readManagedConnection(value: object):
  | {
  ...
  | undefined {
  const candidate = (value as Record<string, unknown>).managedConnection;

  if (!candidate || typeof candidate !== "object") {
    return undefined;
  }

  const sourceEndpoint = readString(candidate, "sourceEndpoint");
  const sourceEntityId = readString(candidate, "sourceEntityId");
  const targetEndpoint = readString(candidate, "targetEndpoint");
  const targetEntityId = readString(candidate, "targetEntityId");

  if (
    (sourceEndpoint !== "start" && sourceEndpoint !== "end") ||
    !sourceEntityId ||
    (targetEndpoint !== "start" && targetEndpoint !== "end") ||
    !targetEntityId
  ) {
    return undefined;
  }
